	...
	while (true)
	{
		...
		if (query.compare(0, string("run ").length(), "run ") == 0)
		{
			...
			{	
				...
				DWORD current = 0;
				GetConsoleMode(out, &current);
				DWORD old = current;
				current |= ENABLE_VIRTUAL_TERMINAL_PROCESSING;
				SetConsoleMode(out, current);
				DWORD all = 0;
				PCWSTR clear = L"\033[2J\033[1;1H";
				WriteConsoleW(out, clear, (DWORD) wcslen (clear), &all, NULL);
				SetConsoleMode(out, old);
			}
			continue;
		}
