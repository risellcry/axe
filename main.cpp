		...
		if (query.compare(0, string("run ").length(), "run ") == 0)
		{
			...
			while (true)
			{
				...
				for (int h = 0; h <= 5; h++)
				{
					for (int i = 0; i <= 15; i++)
					{
						try
						{
							...
							auto pixel = pixels_prj[current];
							if (pixel.first != x or pixel.second != y)
							{
								throw exception();
							}
							putchar('+');
						...
						catch (exception error)
						{
							try
							{
								...
								auto _char = chars_prj[current];
								if (_char.second.first != x or _char.second.second != y)
								{
									putchar('-');
									current++;
									continue;
				...
				for (int i = 0; i <= 99; i++)
				{
					...
					system("clear");
				#elif defined(WIN32) and !defined(UNIX)
					system("cls");
				#else
					system("clear");
				#endif
